...
const Testimonials: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTestimonials = async () => {
      try {
        ...
        const activeTestimonials = Array.isArray(data) 
          ? data
  ...
  // Use testimonials from API if available, otherwise use fallback data
  ...
  return (
    <div className="py-0 bg-[#0d0d0d] overflow-hidden">
      <div className="container mx-auto px-4">
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      zIndex: displayTestimonials.length - Math.abs(index - activeIndex),
        ...
        {/* <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-6 border-t border-white/10 max-w-4xl mx-auto px-4">
          <div className="flex items-center gap-2 text-gray-400 text-sm md:text-base">
            <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
            <span>Click on any card to navigate</span>
          </div>

          <div className="flex items-center text-lg font-semibold">
            <span className="text-primary">{activeIndex + 1}</span>
            <span className="text-gray-500 mx-1">/</span>
            <span className="text-gray-300">{displayTestimonials.length}</span>
          </div>
        </div> */}
      </div>
    </div>
