...
const STATUS_STYLES: Record<string, string> = {
  ...
  verified: 'bg-green-500/15 text-green-400 border border-green-500/20',
...
export default function PaymentsPage() {
  ...
  // Reject dialog
  const [rejectDialog, setRejectDialog] = useState<{ isOpen: boolean; paymentId: string }>({
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      setRejectDialog({ isOpen: false, paymentId: '' });
      setRejectReason('');
      fetchPayments(pagination.page);
    } catch {
                ...
                {payments.map((payment) => (
                  <tr key={payment._id} className="hover:bg-white/5 transition-colors">
                    <td className="px-4 py-3">
                      <div>
                        <p className="text-sm text-white">{payment.registrationApplication?.personal?.fullName}</p>
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${STATUS_STYLES[payment.verificationStatus]}`}>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        ...
                        {payment.verificationStatus === 'pending' && (
                          <>
                            ...
                            <button
                              onClick={() => setRejectDialog({ isOpen: true, paymentId: payment._id })}
                              disabled={actionLoading}
